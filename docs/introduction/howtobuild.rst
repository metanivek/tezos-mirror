...
.. note::
   ...
   * Building the smart rollup node compiles RocksDB, whose ``bindgen``
     step needs ``libclang`` and clang's builtin headers. The package list
     above installs them as ``libclang-dev``. On Fedora, RHEL and Rocky it
     is ``clang-devel``. On macOS the Xcode command line tools or the
     Homebrew ``llvm`` formula provide them. If the build fails with
     ``Unable to find libclang``, set ``LIBCLANG_PATH`` to the directory
     holding it.
