...
(* Previewnet counterpart of [test_kernel_storage_version_matches_constant] in
   ...
   [/evm/storage_version] with V49 forces the migration ladder to run —
   the same trick the retired mainnet-fixture migration tests used; once
   it has run, the kernel writes its
...
let test_previewnet_storage_version_matches_constant =
  ...
  @@ fun protocol ->
  let admin = Constant.bootstrap1 in
  (* V49 as a little-endian u64: the version the migration ladder starts from. *)
